...
if __name__ == "__main__":
    ...
    for v in range(len(file_list)):
        ...
        imgin3 = imgZ.copy()
        imgin4 = imgZ.copy()
        for i in range(2,imgin3.shape[0]-2):
            for j in range(2,imgin3.shape[1]-2):
                

                left = (imgZ[i+1,j]-imgZ[i+1,j-1])**2 + (imgZ[i-1,j]-imgZ[i-1,j-1])**2
                right = (imgZ[i+1,j]-imgZ[i+1,j+1])**2 + (imgZ[i-1,j]-imgZ[i-1,j+1])**2
                down = (imgZ[i,j+1]-imgZ[i+1,j+1])**2 + (imgZ[i,j-1]-imgZ[i+1,j-1])**2
                up = (imgZ[i,j+1]-imgZ[i-1,j+1])**2 + (imgZ[i,j-1]-imgZ[i-1,j-1])**2
                
                otscore = [left,right,down,up]
                leftind = imgZ[i,j-1]
                rightind = imgZ[i,j+1]
                downind = imgZ[i+1,j]
                upind = imgZ[i-1,j]
                
                otcount = [leftind,rightind,downind,upind]
                
                randone = np.argmin(otscore)
                randone = otcount[randone]
                if (i + j) % 2 == 0:
                    imgin3[i,j] = randone
                else:
                    imgin4[i,j] = randone
        ...
        imgupcpunow = torch.from_numpy(imgupcpu).view(1,1,imgin.shape[2],imgin.shape[3])
        bothapcpunow = torch.from_numpy(bothapcpu).view(1,1,imgin2.shape[2],imgin2.shape[3])
