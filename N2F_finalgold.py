...
if __name__ == "__main__":
    ...
    for v in range(len(file_list)):
        ...
        bothap = imgflat_black.view(img.shape)
        imgup =  imgflat_white.view(img.shape)
        ...
        for sp in range(len(ifunctions)):
            last10blind.append(np.zeros_like(img[0,0,:,:].cpu().detach().numpy()))
            last10mask.append(np.zeros_like(img[0,0,:,:].cpu().detach().numpy()))
        ...
        while goodo:
            ...
            if counter % countso == (countso-1) and counter > countso:
                ...
                H0 = H0*whitet
                V0 = V0*blackt
                unfold64 = nn.Unfold(kernel_size=(64, 64), stride=64)
                H0 = torch.from_numpy(H0).view(img.shape).to(device)
                V0 = torch.from_numpy(V0).view(img.shape).to(device)
                
                H0 = unfold64(H0)
                V0 = unfold64(V0)
                
                imgupcpunow = torch.from_numpy(imgupcpu).view(img.shape).to(device)
                bothapcpunow = torch.from_numpy(bothapcpu).view(img.shape).to(device)
                ...
                #print(newval>lastval)
                
                if (np.sum(newval>lastval)/len(newval>lastval)) > 0.8:
                    goodo = False
                ...
                #print(counter+1)
                print(np.mean((GT-H2)**2))
                #print(psnr(GT,H2, data_range=255))
                ...
                #print((np.mean((GT-H2)**2),np.mean((imgup[0,0,:,:].cpu().detach().numpy()*maxer+minner-H)**2)))
