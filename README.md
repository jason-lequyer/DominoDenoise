...
Highly multiplexed imaging techniques are vital tools in biomedical research, used to study complex tissue structures and cellular interactions in both normal and disease states. Despite their capabilities, these techniques often suffer from significant noise, the random fluctuation of pixel intensity, and bleed-through, where signals from different channels interfere with each other. Existing methods for restoring these images are either incapable of adequately dealing with this, or require significant effort to obtain the training sets and data necessary for them to work. Here we present RefineOT, a novel blind zero-shot denoising and bleed-through correction software package that is completely plug-and-play. RefineOT operates unsupervised, learning solely from the input image without the need for training datasets, sensitive hyperparameters, or predefined spillover matrices.  RefineOT features our novel Semi-Blind Spot Network (SBSN) technique for denoising and our innovative Pixel Domino Tiling (PDT) which removes bleed-through by leveraging optimal transport to identify similar patches across channels and retain only the brightest instance of a given signal. We demonstrate that RefineOT surpasses existing zero-shot denoisers and markedly improves the reliability and precision of downstream analyses on Image Mass Cytometry (IMC) and Tissue Cyclic Immunofluorescence (t-CyCIF) datasets. RefineOT's adaptability positions it as a valuable asset for a broad spectrum of applications.
