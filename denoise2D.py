...
if __name__ == "__main__":
    ...
    while notdone:
        ...
        #ParticalConv2D is adapted from https://github.com/JinYize/self2self_pytorch, which is based on https://github.com/NVIDIA/partialconv
        ...
        for v in range(len(file_list)):
            ...
            while goodo:
                ...
                if counter % countso == (countso-1) and counter > countso:
                    ...
                    #print(counter+1)
                    if np.sum(np.round(H2[1:-1,1:-1]-np.mean(H2[1:-1,1:-1]))>0) <= 25 and learning_rate != 0.00005:
                        goodo = False
        ...
        try:
            imwrite(outfolder + '/' + file_name, np.round(H2).astype(typer), imagej=True)
        except:
            imwrite(outfolder + '/' + file_name, np.round(H2).astype(np.float32), imagej=True)
